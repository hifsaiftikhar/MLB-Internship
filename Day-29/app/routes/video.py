...
def error_response(message: str, request_id: str, status_code: int):
    raise HTTPException(
        ...
        detail={"success": False, "error": message, "request_id": request_id}
    )
...
@jobs_router.get("/")
def get_all_jobs(
    status: Optional[str] = Query(default=None, description="Filter by status: queued, processing, completed, failed"),
    db: Session = Depends(get_db)
):
